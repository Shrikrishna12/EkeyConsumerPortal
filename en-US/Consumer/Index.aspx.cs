...
namespace TRAWebApplication.en_US
{
    public partial class Index : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
               ...
               // logger.Error((Request.Params["isLogin"]).ToString());
                if (Convert.ToBoolean(Request.Params["isLogin"]) == true)
                {
                    logger.Error("pointer was here on  line no : 41");
                ...
                if (isCAuth)
                {
                    ...
                    if (ConsumerDetails.AccessToken == null && ConsumerDetails.Error == "invalid_grant")
                    {
                        ...
                        Response.Redirect("/en-us/Consumer/registerConsumer.aspx");
                    }
                    else
                    {
        ...
        protected void cmdPost_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (validateParams)
                {
                    ...
                    {
                        //logger.Error("Error Occured in first catch  block: " + ex);
                        string redirectURL = ResolveUrl("~/error.html");
                        Session["ErrorMessage"] = dr.ErrorMessage;
                        Response.Redirect(redirectURL);
                    }
                    ...
                    {
                        // logger.Error("Error Occured in 2nd catch  block: " + ex);
                        string redirectURL = ResolveUrl("~/error.html");
                        Session["ErrorMessage"] = dr.ErrorMessage;
                        Response.Redirect(redirectURL);
                    }
            ...
            {
                logger.Error("Error Occured in 3rd catch  block: " + ex);
                string redirectURL = ResolveUrl("~/error.html");
                Session["ErrorMessage"] = dr.ErrorMessage;
                Response.Redirect(redirectURL);
            }
